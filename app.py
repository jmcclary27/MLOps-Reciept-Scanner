...
def _ensure_loaded():
    ...
    # Normalize AIP_STORAGE_URI to the parent artifacts folder
    def _normalize_to_artifacts_root(uri: str) -> str:
        u = uri.rstrip("/")
    ...
    if AIP_STORAGE_URI.startswith("gs://"):
        root_uri = _normalize_to_artifacts_root(AIP_STORAGE_URI)
        _download_artifacts_if_needed(root_uri, LOCAL_MODEL_DIR)
        artifacts_root = LOCAL_MODEL_DIR
    else:
        artifacts_root = _normalize_to_artifacts_root(AIP_STORAGE_URI)

    model_dir = os.path.join(artifacts_root, "model")
    proc_dir  = os.path.join(artifacts_root, "components")

    PROCESSOR = TrOCRProcessor.from_pretrained(proc_dir)
